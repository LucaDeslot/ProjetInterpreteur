...
void Lecteur::sauterSeparateurs() {
    while (m_lecteurCar.getCaractere() == ' ' || m_lecteurCar.getCaractere() == '\t'
            || m_lecteurCar.getCaractere() == '\r' || m_lecteurCar.getCaractere() == '\n')
        m_lecteurCar.avancer();
    if (m_lecteurCar.getCaractere() == '#') {
        do {
            m_lecteurCar.avancer();
        } while (m_lecteurCar.getCaractere() != '\r' && m_lecteurCar.getCaractere() != '\n'
...
LecteurCaractere::LecteurCaractere(ifstream & fichier) :
        m_fichier(fichier) {
    m_ligne = 1;
    m_colonne = 0;
